...
int main(int argc, char * argv[]) {
    ...
    int pid = fork();
	if (pid == 0){
        // Pass the request channel type as an arg
        ...
        for(int j = 0; j < w; ++j){
		    ...
		    s = chan->cread();
            RequestChannel *worker_chan = nullptr;
            // Get the RequestChannel based on the control channel
            if(chan->get_type() == 'f'){
                worker_chan = new FIFORequestChannel(s, RequestChannel::CLIENT_SIDE);
            } else if(chan->get_type() == 'q'){
                worker_chan = new MQRequestChannel(s, RequestChannel::CLIENT_SIDE);
            } else if(chan->get_type() == 's'){
                worker_chan = new SHMRequestChannel(s, RequestChannel::CLIENT_SIDE);
            } else {
                worker_chan = new FIFORequestChannel(s, RequestChannel::CLIENT_SIDE);
            } 
            wd[j].workerChannel = worker_chan;
            pthread_create(&worker[j], NULL, worker_thread_function, (void *) &wd[j]);
        }
